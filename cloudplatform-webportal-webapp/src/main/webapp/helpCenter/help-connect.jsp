...
<!-- <div class="content-right" data-spm="2"> -->
    ...
    <div class="article-content">
        <p><span>1. 先选择一个已经创建好的数据库“zhangtest022” -->系统资源监控，可查看到内网地址:10.150.147.203。在这里说明下这个ip地址不能直接用于业务程序连接使用，程序需要连接数据库请参考<a href="" class="help-createUser" data-spm-click="help-createUser.jsp">"创建用户流程"</a>，这个ip地址是专门用于个人办公环境连接数据库地址，把这个ip地址添加到白名单中,如下图：</p>
        <p><img title="" src="${ctx}/static/img/help/18.jpg"></p>
        <p><img title="" src="${ctx}/static/img/help/19.jpg"></p>
        <p><span>2. 【<span style="line-height:1.5;font-family:微软雅黑,Microsoft YaHei;font-size:10.5pt;">账户管理】</span>,右上“创建数据库用户”，用户可根据自身需要创建：管理员用户、读写用户和只读用户，注意：管理用户具有修改表、添加表权限，但是管理用户max_query和max_update并发量不高，读写用户和只读用户可以在下面设置并发量（最大为2000)。如下图：</p>
        <p><img title="" src="${ctx}/static/img/help/20.jpg"></p>
        <p><span>用户创建成功后，连接vip地址 10.150.147.203 3306端口：mysql -uzhang_test3 -p -h10.150.147.203</p>
    </div>
<!-- </div> -->
<script>
