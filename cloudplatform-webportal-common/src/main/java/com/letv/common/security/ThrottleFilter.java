 ...
 */
package com.letv.common.security;
...
public class ThrottleFilter implements Filter, ThrottleFilterMXBean {
    ...
    private static final long PERIOD = 100L; // second
    ...
    public void throttlingJob() {
        if (enable) {
            ...
            synchronized (ThrottleFilter.this) {
                for (Iterator<Map.Entry<String, Integer>> it = ip2countCache.entrySet().iterator(); it.hasNext();) {
                    Map.Entry<String, Integer> ip2count = it.next();
