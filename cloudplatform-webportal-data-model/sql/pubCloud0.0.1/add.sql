...
CREATE TABLE `MATRIX_INVITE_CODE` (
  `ID` bigint(20) unsigned NOT NULL AUTO_INCREMENT COMMENT '邀请码记录表主键',
  `USER_ID` bigint(20) unsigned DEFAULT NULL COMMENT '用户ID',
  `INVITE_CODE` varchar(12) DEFAULT NULL COMMENT '邀请码',
  `USED` tinyint(1) DEFAULT '0' COMMENT '是否使用，1-使用，0-未使用',
  ...
  `DESCN` varchar(200) DEFAULT NULL COMMENT '描述',
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='邀请码记录表'

CREATE INDEX index_inviteCode_userId ON `MATRIX_INVITE_CODE` (USER_ID);
CREATE INDEX index_inviteCode_inviteCode ON `MATRIX_INVITE_CODE` (INVITE_CODE);
