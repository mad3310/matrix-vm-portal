...
@Service("taskLogStartElesticService")
public class TaskLogStartElesticServiceImpl extends BaseTask4LogServiceImpl implements IBaseTaskService{

	@Autowired
	private ILogPythonService logPythonService;
	
	private final static Logger logger = LoggerFactory.getLogger(TaskLogStartElesticServiceImpl.class);
	...
	public TaskResult execute(Map<String, Object> params) throws Exception {
		...
		if(!(Boolean) params.get("isCreateLog"))  {
			tr.setSuccess(true);
			tr.setResult("no need to start elesticSearch");
			return tr;
		}
		//执行业务
		...
		ApiResultObject result = this.logPythonService.startElesticSearch(nodeIp1,port, cluster.getAdminUser(), cluster.getAdminPassword());
