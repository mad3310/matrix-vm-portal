/*
 * @Title: MemcachedCacheManagerImpl.java
 * @Package com.letv.mms.cache.client.memcached
 * @Description: TODO
 * @author chenguang 
 * @date 2012-12-18 下午6:23:38
 * @version V1.0
 *
...
import cache.memcached.MemcachedClientConfig;
import cache.memcached.MemcachedClientSocketPoolConfig;
import cache.memcached.main.MemCachedClient;
import cache.memcached.main.SockIOPool;
 ...
 *
 */
public class MemcachedCacheManagerImpl implements ICacheManager<IMemcachedCache>
{
	private static Logger logger =
			Logger.getLogger(MemcachedCacheManagerImpl.class);
	/**
	 * 配置文件名称
	 */
			...
			if (supportMultiConfig)
			{
		    	...
		    	while(urls.hasMoreElements())
		    	{
		    		...
		    		CacheUtil.loadMemcachedConfigFromURL(url,factory,
		    				memcachedClientconfigs,memcachedClientSocketPoolConfigs);
	...
	protected void initMemCacheClientPool()
	{
		...
		for(MemcachedClientSocketPoolConfig socketPool : memcachedClientSocketPoolConfigs)
		{
					...
					socketPool.getServers() != null && !socketPool.getServers().equals(""))
			{
				...
				if (socketpool.get(socketPool.getName())!= null){
					logger.error("socketpool define duplicate! socketpool name:"+socketPool.getName());
				}
					
				
				socketpool.put(socketPool.getName(), pool);
				logger.info(" add socketpool :"+socketPool.getName());
		...
		for(MemcachedClientConfig node : memcachedClientconfigs)
		{
			...
			IMemcachedCache cache = new MemcachedCacheImpl(helper,responseStatInterval);
			...
			try
			{
				if (node.getErrorHandler() != null && !node.getErrorHandler().equals(""))
					client.setErrorHandler((IErrorHandler)Class.forName(node.getErrorHandler()).newInstance());
				
			}
			catch(Exception ex)
			{
				logger.error("Not find class name:"+node.getErrorHandler()+"please check space char or tab char");
			}
			...
			if (cachepool.get(node.getName())!= null){
				logger.error("cache define duplicate! cache name :"+node.getName());
			}
			cachepool.put(node.getName(), cache);
			logger.info(" add memcachedClient :"+node.getName());
		}
